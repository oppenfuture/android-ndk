...
namespace scene
{
	...
	*/
	...
	public:
        ...
        IsVisible(true), IsDebugObject(false), RenderLayer(RenderQueue::Normal)
		{
			if (parent)
		...
		*/
		...
		void TransformVector(core::vector3df& v)
		{
			f32 vector[4];
			v.getAs4Values(vector);
			updateAbsolutePosition();
			AbsoluteTransformation.multiplyWith1x4Matrix(vector);
			v.X = vector[0];
			v.Y = vector[1];
			v.Z = vector[2];
	}

		void TransformPoint(core::vector3df& v)
		{
			f32 p[4];
			v.getAs4Values(p);
			p[3] = 1;
			updateAbsolutePosition();
			AbsoluteTransformation.multiplyWith1x4Matrix(p);
			v.X = p[0];
			v.Y = p[1];
			v.Z = p[2];
		}
        ...
        //set render queue for object
        virtual void setRenderQueue(RenderQueue queue)
        {
            RenderLayer = queue;
            ISceneNodeList::Iterator it = Children.begin();
            for (; it != Children.end(); ++it)
                (*it)->setRenderQueue(queue);
        }
