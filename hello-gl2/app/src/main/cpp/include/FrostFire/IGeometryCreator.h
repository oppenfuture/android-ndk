...
public:
	...
	*/
	...
	virtual IMesh* createQuadMesh(const f32& width = 1, const f32& height = 1) const = 0;

	virtual IAnimatedMesh* createCustomMesh(const float* vertices,const uint32_t& vertices_count,  const uint32_t* indices,const uint32_t& indices_count) const =0;
};
